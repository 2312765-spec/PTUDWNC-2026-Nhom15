...
| FR-RCP-008 | Quản lý ảnh | M | `POST /recipes/{id}/images` · `PATCH .../{imageId}` · `DELETE .../{imageId}` | S8 | `UploadRecipeImageCommand`, `UpdateRecipeImageCommand`, `DeleteRecipeImageCommand` | `Recipe.AttachImage/UpdateImage/RemoveImage`, `MinioFileStorageService`, magic-byte validation | `Recipes/ImagesTests.cs` (16 test, gồm hồi quy đổi primary qua lại + upload đồng thời) + 6 file `UnitTests/Images` + `UnitTests/Domain/RecipeImageTests.cs`. **FE:** `components/upload/*`, `lib/hooks/useRecipeImage{s,Gallery}.ts` — `__tests__/components/upload/*`, `__tests__/lib/**` (66 test Jest) | **D22, D27, D28, D30, D31**, D16 | ✅ |
